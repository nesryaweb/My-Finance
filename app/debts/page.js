...
export default function DebtsPage() {
  ...
  const [deletingReceivedId, setDeletingReceivedId] = useState(null);
  ...
  async function loadDebts() {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.error || "Failed to load debts",
        );
      }
  ...
  async function deleteDebt(debt) {
    ...
    const hasReceivedHistory =
      debt.received?.length > 0;
  ...
  async function deleteReceivedMoney(
    debt,
    received,
  ) {
    const confirmed = window.confirm(
      `Are you sure you want to delete ${Number(
        received.amount,
      ).toLocaleString()} birr received on ${new Date(
        received.date,
      ).toLocaleDateString()}?\n\n` +
        "This will also remove the related transaction and the money will no longer appear in the account balance.\n\n" +
        "This action cannot be undone.",
    );
    ...
    try {
      ...
      const response = await fetch(
        `/api/debts/${debt.id}/receive/${received.id}`,
        {
          method: "DELETE",
        },
      );
  ...
  return (
    <div className="mx-auto w-full max-w-6xl p-6">
      ...
      {/* ==================================================
          EDIT RECEIVED MONEY MODAL
      ================================================== */}

      {editingReceived && (
        <EditReceivedMoneyModal
          debt={editingReceived.debt}
          received={
            editingReceived.received
          }
          onClose={() =>
      ...
      {!isPaid && (
        <div className="mt-5 flex flex-wrap gap-2">
          ...
          <button
            onClick={onDelete}
            disabled={deleting}
            className="cursor-pointer rounded-md border border-destructive/30 px-4 py-2 text-sm font-medium text-destructive hover:bg-destructive/10 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {deleting
              ? "Deleting..."
              : "Delete debt"}
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex shrink-0 gap-2">
                        <button
                          ...
                          className="cursor-pointer rounded-md border px-2.5 py-1.5 text-xs font-medium disabled:cursor-not-allowed disabled:opacity-50"
                        >
                          Edit
                        </button>
                        ...
                        <button
                          ...
                          className="cursor-pointer rounded-md border border-destructive/30 px-2.5 py-1.5 text-xs font-medium text-destructive hover:bg-destructive/10 disabled:cursor-not-allowed disabled:opacity-50"
                        >
                          {isDeleting
                            ? "Deleting..."
                            : "Delete"}
...
function ReceiveDebtModal({
  ...
  async function receive() {
    if (!accountId) {
      setError(
        "Please select the account where you received the money.",
      );
      return;
    }
    ...
    if (
      Number(amount) >
      Number(debt.remainingToReceive)
    ) {
      setError(
        `You can only receive ${Number(
          debt.remainingToReceive,
        ).toLocaleString()} birr more for this debt.`,
      );
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.error ||
            "Failed to record borrowed money",
        );
      }
  ...
  return (
    <Modal>
      ...
      <div className="mb-5 rounded-lg bg-muted p-4">
        ...
        <p className="mt-2 text-xs text-muted-foreground">
          This will add the received amount to the selected account. It will not be counted as income.
        </p>
      </div>
...
function EditReceivedMoneyModal({
  debt,
  received,
  onClose,
  onSaved,
    ...
    <Modal>
      <div className="mb-6 flex items-start justify-between">
        ...
        <button
          onClick={onClose}
          disabled={saving}
          className="cursor-pointer text-xl text-muted-foreground disabled:cursor-not-allowed disabled:opacity-50"
        >
          ×
        </button>
      </div>
      ...
      <div className="mb-5 rounded-lg bg-muted p-4">
        ...
        <p className="mt-2 text-xs text-muted-foreground">
          Editing this record will also update the related account transaction.
        </p>
      </div>
      ...
      <div className="mt-6 flex justify-end gap-2">
        ...
        <button
          ...
          className="cursor-pointer rounded-md bg-primary px-4 py-2 text-sm text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50"
        >
          {saving
            ? "Saving..."
            : "Save changes"}
